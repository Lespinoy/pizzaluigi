...
public class VoorkeurPizzaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if (request.getParameter("id") != null){
			request.setAttribute("voorkeurPizzas", Arrays.stream(request.getParameterValues("id")).map(id -> pizzaRepository.read(Long.parseLong(id))).filter(optionalPizza -> optionalPizza.isPresent()).map(optionalPizza ->optionalPizza.get()).collect(Collectors.toList()));
		}
